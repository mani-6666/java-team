...
export default function AdminAnalytics() {
  ...
  async function fetchUserEngagement() {
    try {
      const res = await fetch(
        `${API_BASE}/admin/analytics/stats/user-engagement`,
        { credentials: "include" }
      );
      ...
      if (import.meta.env.DEV) console.error("fetchUserEngagement:", err);
    }
  }
  ...
  async function fetchExamStats() {
    try {
      const res = await fetch(
        `${API_BASE}/admin/analytics/stats/exams`,
        { credentials: "include" }
      );
      ...
      if (import.meta.env.DEV) console.error("fetchExamStats:", err);
    }
  }
  ...
  async function fetchUserStats() {
    try {
      const res = await fetch(
        `${API_BASE}/admin/analytics/stats/users`,
        { credentials: "include" }
      );
      ...
      if (import.meta.env.DEV) console.error("fetchUserStats:", err);
    }
  }
  ...
  async function fetchActivity(range) {
    try {
      const res = await fetch(
        `${API_BASE}/admin/analytics/stats/activity?range=${encodeURIComponent(
          range
        )}`,
        { credentials: "include" }
      );
      ...
      if (import.meta.env.DEV) console.error("fetchActivity:", err);
      setActivityData([]);
    }
  }
        ...
        <div className="bg-white dark:bg-[#0f0f0f] rounded-xl p-6 shadow border border-gray-100">
          ...
          <div className="space-y-3">
            ...
            <p className="text-gray-600 text-sm">
              Invigilators
              <span className="float-right font-semibold">
                {userStats.total_invigilators ?? 0}
              </span>
            </p>
            <p className="text-gray-600 text-sm">
              Active Users
