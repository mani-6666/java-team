...
const API_BASE = import.meta.env.VITE_API_BASE_URL + "/admin/study-materials";
...
export default function AdminStudyMaterials() {
  ...
  async function fetchMaterials() {
    try {
      ...
      if (import.meta.env.DEV) console.error("Fetch materials error:", err);
      setMaterials([]);
    } finally {
      setLoading(false);
    }
  ...
  async function uploadToFirebase(file, category, fileType) {
    ...
    const typeFolder = fileType || "Others";
    ...
    return new Promise((resolve, reject) => {
      uploadTask.on(
        ...
        () => {},
        (error) => {
          if (import.meta.env.DEV) console.error("Firebase upload error:", error);
          reject(error);
        },
        async () => {
  ...
  async function deleteFromFirebase(path) {
    ...
    try {
      ...
      if (import.meta.env.DEV) console.error("deleteFromFirebase:", err);
    }
  }
